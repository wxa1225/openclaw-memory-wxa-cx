...
### 5. LLM-Enhanced Proactive Memory Capture

两阶段 AI 架构：
- **第一阶段：正则快速检测**（毫秒级，零延迟）— 10 种触发模式实时扫描
- **第二阶段：LLM 结构化提取**（异步，增强质量）— 调用大模型做 entity/attribute/value 精确提取 + 分类 + 置信度评分
- 飞书卡片交互：确认/忽略/编辑后保存
...
| 维度 | 本方案 | Mem0 | 传统 Wiki |
|------|--------|------|----------|
| **遗忘管理** | ✅ Ebbinghaus 曲线 + 自适应半衰期 | ❌ 无 | ❌ 无 |
| **版本管理** | ✅ 完整版本链 + 冲突检测 | ❌ 无 | ⚠️ Git 历史 |
| **风险评估** | ✅ 5 维模型 + 双阈值 | ❌ 无 | ❌ 无 |
| **知识分布** | ✅ TMS 追踪"谁知道什么" | ❌ 无 | ❌ 无 |
| **离职模拟** | ✅ 量化影响 + 传承推荐 | ❌ 无 | ❌ 无 |
| **自动捕获** | ✅ 正则+LLM 两阶段 | ⚠️ 仅 LLM | ❌ 手动 |
| **飞书集成** | ✅ 卡片交互 + WebSocket | ❌ 无 | ❌ 无 |

**差异化壁垒**：
1. 遗忘曲线模型是**学术级实现**（9 级间隔 + 自适应半衰期 + 复习记录），不是简单的 TTL
2. TMS + 离职模拟是**组织行为学理论**落地，市面上没有竞品
3. 版本链 + 冲突检测 = 记忆的 Git，支持团队协作场景
...
| 测试 | Jest（13 个测试文件，247 个测试用例 + 集成测试 24 个） |
...
| 单元测试 | 247/247 pass |
| 集成测试 | 24/24 pass |
| 并发写入 | 20/20 条完整保留（writeLock 保护） |
| Graph 构建 | 88 节点 + 226 边（30 条记忆） |
| 数据规模 | 30 条记忆 / 6 类别 / 5 人团队 |

### 对比传统方案

| 场景 | 传统 Wiki | 本方案 |
|------|-----------|--------|
| 决策记录 | 手动写文档 | AI 自动提取，准确率 >90% |
| 知识流失 | 无法量化 | 离职模拟精确量化影响（%） |
| 遗忘风险 | 无感知 | 5 维风险评分，自动提醒复习 |
| 新人上手 | 需要人工介绍 | 记忆图谱可视化，即时了解全貌 |
| 冲突检测 | 无 | 版本链 + 飞书卡片实时通知 |
