...
如果是本地仓库调试，请先打包再安装，不要直接把整个仓库目录传给
OpenClaw。这个仓库里还包含了其他插件项目和工具文件，直接安装仓库根目录
时容易触发 OpenClaw 的安全扫描拦截。
...
插件现在支持两种配置位置：
- `plugins.entries["openclaw-mem0-plugin"].config`（兼容旧写法）
- 环境变量，即通过 `process.env` 或 `api.config.env` 读取
...
如果不想把配置放在插件的 `config` 里，也可以通过环境变量提供：
