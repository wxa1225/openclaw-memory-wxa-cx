...
export class MemoryClient {
  ...
  async update(memoryId: string, { text, metadata }: { text?: string, metadata?: Record<string, any> }) {
    ...
    console.log(
      `openclaw-mem0-plugin: update memory request memoryId:${memoryId}, text:${payload.text}, metadata:${JSON.stringify(payload.metadata)}`
    );
    return response;
  }
  ...
  async getAll(options: Record<string, any> = {}) {
    console.log(
      `openclaw-mem0-plugin: getAll memory request options:${JSON.stringify(options)}`
    );
    this._validateOrgProject();
  ...
  async search(query: string, options: Record<string, any> = {}) {
    console.log(
      `openclaw-mem0-plugin: search memory request query:${query}, options:${JSON.stringify(options)}`
    );
    this._validateOrgProject();
  ...
  async delete(memoryId: string) {
    console.log(
      `openclaw-mem0-plugin: delete memory request memoryId:${memoryId}`
    );
    return this._fetchWithErrorHandling(
      `${this.host}/v1/memories/${memoryId}/`,
      {
  ...
  async deleteAll(options: Record<string, any> = {}) {
    console.log(
      `openclaw-mem0-plugin: deleteAll memory request options:${JSON.stringify(options)}`
    );
    this._validateOrgProject();
    if (this.organizationName != null && this.projectName != null) {
      options.org_name = this.organizationName;
  ...
  async history(memoryId: string) {
    console.log(
      `openclaw-mem0-plugin: history memory request memoryId:${memoryId}`
    );
    const response = await this._fetchWithErrorHandling(
  ...
  async getWebhooks(data?: { projectId?: string }) {
    console.log(
      `openclaw-mem0-plugin: getWebhooks memory request projectId:${data?.projectId || this.projectId}`
    );
  ...
  async createWebhook(webhook: any) {
    console.log(
      `openclaw-mem0-plugin: createWebhook memory request webhook:${JSON.stringify(webhook)}`
    );
    const response = await this._fetchWithErrorHandling(
  ...
  async updateWebhook(webhook: { webhookId: string, projectId?: string, [key: string]: any }) {
    console.log(
      `openclaw-mem0-plugin: updateWebhook memory request webhook:${JSON.stringify(webhook)}`
    );
  ...
  async deleteWebhook(data: { webhookId?: string } | string) {
    console.log(
      `openclaw-mem0-plugin: deleteWebhook memory request webhookId:${typeof data === 'string' ? data : data.webhookId}`
    );
